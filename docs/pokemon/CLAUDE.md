...
`sim/` runs the full 165-deck field in ~20s. Core rules + a **5-layer effect registry** (`effects.py`) are implemented and unit-verified: **(1) energy acceleration**, **(2) scaling damage** (×/+/− with conditions + attack side-effects), **(3) special conditions** (burn/poison/sleep/paralysis/confusion + between-turns checkup), **(4) heal / damage reduction / HP buffs / immunity** (`incoming_damage`, `team_hp_bonus`, `HEAL_ABILITIES`), and **(5) spread + ability-lock** (`apply_spread`, `abilities_disabled`). Piloting has been upgraded to full-deck fidelity: realistic auto-lists (`decks_build.py` returns each deck's *ace*), an **ace-aware AI** (funds/evolves the ace incl. on the bench, retreats/Switches to promote the readiest attacker, KO/value/gust attack selection), **utility attacks** (draw/search), and **Trainer cards**. Trainer data comes from `carddata/*.json` (the raw source `types.html` was filtered from) — 197 legal Common/Uncommon Trainers extracted by `deckgen/parse_trainers.py` → `deckgen/trainers.json`. The engine plays a core toolbox (Professor's Research/Cheren/Judge draw, Boss's Orders gust, Rare Candy, Buddy-Buddy Poffin/Poké Ball search, Switch, Earthen Vessel/Energy Search, Night Stretcher), and `decks_build.trainer_package` gives every deck a consistency shell. Per-deck Trainer tech is done: a **generic text-driven Trainer resolver** (`engine._tcat` + `_do_*`: draw/gust/search-poke/search-energy/accel/heal/rare-candy/switch/recover) plus an **archetype-aware selector** (`decks_build.trainer_package`): universal core + Stage-2→Rare Candy+Dawn, Mega→Mega Signal, multi-color→Crispin, high-cost mono→type energy card, tanky→Pokémon Center Lady, and each named family→its engine (Ethan's Adventure, Arven's Sandwich, Hop's Bag, Janine's, Team Rocket's Great Ball/Transceiver). **Result: a balanced full-deck field** — median ~50%, 133/165 in a 30–70% band, 0 draws, tight cross-archetype matchups. Remaining polish (optional): Stadiums/Tools, deeper AI (sequencing, gust target choice), and a few structurally weak archetypes (2-color Multi like Salamence, slow Metal). The sim now meets the goal of accurately pitting complex deck constructions against each other.
