...
`sim/` runs the full 165-deck field in ~20s. Core rules + a **5-layer effect registry** (`effects.py`) are implemented and unit-verified: **(1) energy acceleration**, **(2) scaling damage** (×/+/− with conditions + attack side-effects), **(3) special conditions** (burn/poison/sleep/paralysis/confusion + between-turns checkup), **(4) heal / damage reduction / HP buffs / immunity** (`incoming_damage`, `team_hp_bonus`, `HEAL_ABILITIES`), and **(5) spread + ability-lock** (`apply_spread`, `abilities_disabled`). Piloting has since been upgraded: realistic auto-lists (`decks_build.py` returns each deck's *ace*), an **ace-aware AI** (funds/evolves the ace incl. on the bench, retreats to promote the readiest attacker, KO/value attack selection), and **utility attacks** (draw/search). Validated: 0 draws, ~120/165 decks in a 30–70% band. The dominant remaining factor is **structural — the pool is Pokémon-only (no Trainers)**, so fast Basic/accel decks (Lightning ~80%, Fighting ~74%) beat singleton Stage-2 evolution decks (Grass ~34%) that can't set up. Accurate for a *trainerless* format, but it under-represents complex/evolution decks. **Biggest next lever: a generic "consistency shell"** — a standard abstract Trainer engine (draw supporter, Pokémon search, switch, Rare-Candy stage-skip) added to every deck — since the data has no Trainer cards. Then niche effects + AI polish.
