...
`sim/` runs the full 165-deck field in ~20s. Core rules + a growing **effect registry** (`effects.py`) are implemented: **(1) energy acceleration**, **(2) scaling damage** (×/+/− with conditions + attack side-effects), and **(3) special conditions** (burn/poison/sleep/paralysis/confusion with a between-turns checkup). Still **not** modeled — so those decks are under-valued, not weak: **(4) healing + damage reduction / HP buffs / immunity** (wall & heal decks), **(5) spread + gust + disruption / ability-lock**, plus a few remaining accel abilities and scaling patterns. Next step: build layers 4–5 (see `sim/README.md` "Extending"), re-running `sim_run.py --all` after each.
