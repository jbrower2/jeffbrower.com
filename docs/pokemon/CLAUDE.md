...
`sim/` runs the full 165-deck field in ~20s. Core rules + a **5-layer effect registry** (`effects.py`) are implemented and unit-verified: **(1) energy acceleration**, **(2) scaling damage** (×/+/− with conditions + attack side-effects), **(3) special conditions** (burn/poison/sleep/paralysis/confusion + between-turns checkup), **(4) heal / damage reduction / HP buffs / immunity** (`incoming_damage`, `team_hp_bonus`, `HEAL_ABILITIES`), and **(5) spread + ability-lock** (`apply_spread`, `abilities_disabled`). The frontier is now **decklist & AI quality, not effects**: setup-heavy decks (Vibrant Wall, Flutter Lock, Symphonia, TR Condition Stack) stay under-valued because the heuristic, trainer-less, energy-heavy auto-lists + greedy AI can't reliably assemble a 2–3-line engine. Next, in order: (a) better `decks_build.py` line/energy ratios, (b) smarter `engine.ai_main` (retreat, target/attack selection, setup-attack priority), (c) niche effects (gust targeting, per-condition scaling, search-accel like Punk Up/X-Boot, disruption).
